document.addEventListener('DOMContentLoaded', () => {
    ...
    restartQuizBtn.addEventListener('click', resetQuiz);
     ...
     // ✅ Fitur Simpan ke localStorage & Kirim ke Server (PHP Laravel)
    saveDataBtn.addEventListener('click', () => {
        ...
        fetch('/save-quiz-data', { // <-- Pastikan ini sesuai dengan rute Laravel
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]') ? document.querySelector('meta[name="csrf-token"]').content : '', // Mengambil CSRF token
                'Accept': 'application/json' // Tambahkan ini agar Laravel mengembalikan JSON response
            },
        ...
        .then(res => {
            ...
            if (contentType && contentType.indexOf("application/json") !== -1) {
                ...
                return res.text().then(text => { throw new Error('Server did not return JSON: ' + text) });
            }
        })
        .then(data => {
        ...
        .catch(err => {
            ...
            alert("Gagal mengirim data ke server. Lihat konsol untuk detail.");
        });
    });
});